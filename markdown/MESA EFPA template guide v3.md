# MESA EFPA Template User Guide v3
...
V3 replaces generic examples with examples from a fixed ten-benchmark bank wherever a stable, source-grounded example is available. Inline examples use short source tags from the Benchmark Example Bank below. Rankings, model counts, and live leaderboard positions are date-sensitive; use them only when the review date and source snapshot are recorded. Part B rating examples are illustrative patterns only, not new review ratings for the named benchmarks.
...
| :--- | :--- | :--- | :--- |
| HLE | Humanity's Last Exam | Public repository documents HLE as a multimodal closed-ended academic benchmark with 2,500 questions across dozens of subjects, multiple-choice and short-answer formats, a Hugging Face dataset link, MIT license, canary string, simple evaluation scripts, contact channel, and accuracy plus calibration reporting. The local HLE pilot remains the strongest source for MESA caution examples about expert-level academic claims and AGI overinterpretation. | https://github.com/centerforaisafety/hle; `markdown/MESA pilot - Humanitys Last Exam.md` |
...
### Omitted Benchmark Alternates

The broader benchmark screenshot/list contained useful alternates, but v3 intentionally limits inline examples to the ten profiles above. Omitted alternates can still be useful in later guides when they add a genuinely new measurement surface; for this version they mostly overlap with selected examples: academic QA overlaps with HLE, ARC-AGI-2, and MMMU-Pro; code generation overlaps with SWE-Bench Verified; browser or computer-use tasks overlap with ScreenSpot-Pro and tau2-bench; factuality overlaps with FACTS; multilingual fairness overlaps with Global PIQA; and long-horizon or agentic simulations overlap with Vending-Bench 2.
...
| Threshold | A decision boundary or score band with an intended interpretation. | Structural note: no v3 bank benchmark is safety-first; use this only when the reviewed source defines a cutoff or score band. |
...
| General capability or intelligence | The benchmark claims broad overall capability, intelligence, or AGI-relevant performance. | ARC-AGI-2 explicitly frames its abstract-reasoning tasks as AGI-relevant. [ARC-AGI-2] |
| Reasoning | Tasks claim inference, logic, multi-step problem solving, or abstract reasoning. | HLE graduate-level science reasoning questions. [HLE] |
| Knowledge | Tasks test factual, academic, professional, or world knowledge. | HLE academic questions. [HLE] |
| Language understanding | Tasks test reading, comprehension, entailment, summarization, or linguistic interpretation. | FACTS Grounding includes long-context user requests whose answers must follow the provided document. [FACTS] |
| Writing or generation quality | Tasks judge style, coherence, creativity, helpfulness, or long-form generation. | Structural note: use only when a reviewed source evaluates writing quality itself, not merely correctness or grounding. |
| Mathematics | Tasks target arithmetic, algebra, proof, contest math, or quantitative reasoning. | HLE includes mathematics among its academic subject areas. [HLE] |
...
| Domain-specific professional expertise | Tasks target law, medicine, finance, cybersecurity, or other professional domains. | HLE professional medicine subject questions. [HLE] |
| Multimodal perception | Tasks require interpreting images, charts, diagrams, or mixed media. | MMMU-Pro tests image/text integration and a vision-only setting. [MMMU-Pro] |
| Audio or speech | Input or output requires speech, audio events, transcription, or spoken interaction. | audio-first benchmark ASR task. [Structural note: no v3 bank benchmark is audio-first] |
...
| Planning or agency | Tasks require multi-step goal pursuit, state tracking, delegation, or autonomous action. | Vending-Bench 2 requires stateful business decisions over a simulated year. [Vending-Bench 2] |
...
| Embodied or simulated environment interaction | Systems act in games, robotics simulators, or virtual environments. | Vending-Bench 2 is a simulated business environment, not a robotics benchmark. [Vending-Bench 2] |
| Safety, refusal, or policy compliance | Tasks evaluate harmful-content handling, refusal, jailbreak robustness, or policy adherence. | Structural note: no v3 bank benchmark is safety-first; use a named safety benchmark only when the reviewed source supports it. |
| Robustness | Tasks test performance under perturbations, distribution shift, adversarial prompts, or stressors. | MMMU-Pro is designed as a more robust variant of MMMU. [MMMU-Pro] |
...
| Leaderboard ranking | Rank submitted systems publicly or privately. | FACTS public ranking table. [FACTS] |
...
| Safety evaluation | Assess risk, compliance, misuse, or refusal behavior. | Structural note: no v3 bank benchmark is safety-first; cite a separate safety benchmark only if it is actually reviewed. |
| Deployment gating | Decide whether a system may be shipped or used. | Structural note: no v3 bank example validates deployment gates; do not infer gating use from leaderboard scores alone. |
...
| Public communication or marketing | Communicate model capability to the public. | Blog post headline using benchmark rank. [FACTS] |
...
| Base language models | Raw next-token models without chat alignment are target systems. | Structural note: no v3 bank benchmark is base-LM-first; use only when the source states base-model evaluation. |
...
| Audio or speech models | Speech-to-text, text-to-speech, audio QA, or spoken agents. | Structural note: no v3 bank benchmark is audio-first; mark only if the reviewed source evaluates audio or speech systems. |
...
| Other | Another target class is documented. | Structural note: name the specific target system class when it falls outside the listed types. |
...
| Safety evaluators | Reviewers focused on harmful behavior or risk. | Structural note: no v3 bank benchmark is safety-first; mark only if the source names safety users or risk evaluation. |
...
| Tool call | Emit tool-call syntax or structured tool invocation. | tau2-bench tool-use benchmark. [tau2-bench] |
...
| File, document, or artifact generation | Produce files such as reports, spreadsheets, slides, or apps. | Structural note: no v3 bank benchmark is office-artifact-first; name the reviewed artifact benchmark if applicable. |
...
| Audio output | Generate speech, music, or audio. | Structural note: no v3 bank benchmark is audio-output-first. |
| Video output | Generate or edit video. | Structural note: Video-MMMU is video understanding, not video generation. |
| Interaction in a simulated environment | Act in a game, simulator, or virtual world. | Vending-Bench 2 is a simulated business environment. [Vending-Bench 2] |
| Interaction in a real or external environment | Act in real services, robots, or live systems. | Structural note: V3 examples cover simulated or tool-mediated environments, not live robotics or production systems. |
| Other | Any documented response mode not listed. | Database transaction, email draft. [tau2-bench] |
...
| Ranking | Order options by preference, quality, risk, or relevance. | Structural note: no v3 bank benchmark is ranking-response-first. |
| Classification | Assign a label. | FACTS judge outputs can classify response eligibility and groundedness. [FACTS] |
| Pairwise preference | Choose better of two outputs or actions. | Structural note: no v3 bank benchmark is pairwise-preference-first. |
...
| Real or external environment task | Interacts with live external systems. | Structural note: no v3 bank example requires acting in a real external environment. |
...
| Audio | Speech, sound, music, acoustic signal. | Structural note: no v3 bank benchmark is audio-first. |
| Video | Moving visual input. | Video understanding task. [Video-MMMU] |
| Documents | PDFs, Word files, slides, spreadsheets. | Structural note: no v3 bank benchmark is document-QA-first; FACTS uses long context documents but not necessarily file artifacts. [FACTS] |
...
| Dynamic environment | State changes as the model acts. | Game or simulator task. [ScreenSpot-Pro] |
| Other | Any other stimulus. | Sensor stream. [ScreenSpot-Pro] |
...
| Development or example items | Items intended for practice, tuning, or format familiarization. | Dev set examples. [FACTS] |
...
| Leaderboard submission | Users submit outputs or systems for ranked scoring. | Public leaderboard upload. [FACTS] |
...
| Interactive agent evaluation | Model acts step by step in an environment. | tau2-bench conversation/tool episodes or Vending-Bench 2 business runs. [tau2-bench; Vending-Bench 2] |
...
| Some control over submitted systems | Some metadata or submission checks exist. | Leaderboard requires model card and logs. [FACTS] |
...
| Other | Any other technology. | Structural note: name the specific simulator, API, or platform if it is outside the listed technologies. |

### 2.14 Time and Resource Requirements

| Activity | Meaning | Example |
| :--- | :--- | :--- |
| Setup | Installation, data download, account setup, environment prep. | Docker build time. [SWE-Bench Verified] |
| Model inference or task completion | Time or compute for answering tasks. | Hours for full model run. [SWE-Bench Verified] |
| Scoring | Time to parse, judge, execute tests, or aggregate results. | Unit tests over generated code. [SWE-Bench Verified] |
| Human adjudication | Time for raters, experts, or reviewers. | HLE human expert review and audit time. [HLE] |
| Analysis or reporting | Time to generate tables, dashboards, or error analysis. | Report script runtime. [FACTS] |
| Full benchmark run | End-to-end total. | SWE-Bench Verified full-run resource cost and time. [SWE-Bench Verified] |
| API or compute cost | Monetary or compute budget. | Dollars per run, GPU-hours. [tau2-bench] |
...
| Adaptive routing based on performance | Later tasks depend on earlier performance. | Structural note: no v3 bank benchmark is primarily adaptive-routing-based. |
...
| Model intermediate trace | Reasoning or scratchpad affects scoring. | Structural note: no v3 bank example makes hidden reasoning traces a primary scoring source; record traces only when scored. |
...
| Prompt | Meaning | Example |
| :--- | :--- | :--- |
| Broad claim term used | Exact broad wording. | "AGI", "general intelligence", "expert capability." [HLE] |
| Claimed breadth across domains | Range of domains claimed. | HLE spans many academic subjects. [HLE] |
| Claimed depth or proficiency level | Claimed difficulty or expertise level. | Expert-level closed-ended academic questions. [HLE] |
| Sub-abilities explicitly covered | Abilities actually measured. | HLE covers knowledge, closed-ended reasoning, and some image comprehension; ARC-AGI-2 covers grid abstraction. [HLE; ARC-AGI-2] |
| Sub-abilities explicitly excluded or not tested | Abilities the source excludes or does not measure. | HLE cautions against inference to autonomous research, open-ended creativity, or long-term planning. [HLE] |
| Evidence across modalities, tools, memory, planning, or speed | Whether broader ability components are represented. | HLE includes some images but not standard tool-use or agency tasks; Vending-Bench 2 covers persistent state and long-horizon actions. [HLE; Vending-Bench 2] |
...
| Relation to CHC-like broad/narrow abilities, if claimed | Any explicit or reviewer-noted mapping to broad/narrow abilities. | "No explicit CHC mapping" for HLE; add reviewer mapping only as a note, not as an author claim. [HLE] |
| Descriptive caution for Part B | A note to later evaluators about claim scope. | Rate against narrow expert academic capability unless broader evidence is supplied. [HLE] |
...
| Automated semantic or embedding-based scoring | Automated similarity or semantic metric scores responses. | Structural note: no v3 bank benchmark is embedding-similarity-first; cite the actual scorer if used. |
...
| Win rate | Percent of pairwise comparisons won. | Structural note: no v3 bank benchmark is pairwise-win-rate-first. |
| Elo or ranking score | Relative rating from pairwise or competition results. | Structural note: no v3 bank benchmark is Elo-first; do not recast ordinary leaderboard rank as Elo. |
| Pairwise preference score | Score from head-to-head preference comparisons. | Structural note: no v3 bank benchmark is pairwise-preference-first. |
...
| Pairwise-comparison based | Organized by head-to-head outcomes. | Structural note: no v3 bank benchmark is pairwise-comparison-first. |
...
| Safety evaluators | Users focused on safety, refusal, harm, or risk. | Structural note: no v3 bank benchmark is safety-first; use this recipient only when the benchmark source names safety users or risks. |
